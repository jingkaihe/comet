...
describe('terminal socket message handling', () => {
  ...
  it('drains every terminal response generated by a PTY output chunk', () => {
    const { pane } = createPane();
    const sent: string[] = [];
    const socket = {
    ...
    const responses = ['\x1b[1;1R', '\x1b]4;0;rgb:00/00/00\x1b\\'];

    pane.socket = socket;
    pane.replayCompleteReceived = true;
    pane.suppressInput = false;
    pane.terminal.wasmTerm = {
      readResponse() {
    ...
    expect(sent).toEqual([
      JSON.stringify({ type: 'input', data: '\x1b[1;1R' }),
